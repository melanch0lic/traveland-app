...
class SentWriteReviewButtonHousing extends StatelessWidget {
  final int providerTypeValue;
  const SentWriteReviewButtonHousing({Key? key, required this.providerTypeValue}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isFilled = context.select((WriteReviewPageHousingViewModel model) => model.isFilled);
    return SizedBox(
      height: 48,
      child: ElevatedButton(
        ...
        onPressed: isFilled
            ? () {
                context.read<WriteReviewPageHousingViewModel>().onSentReviewButtonPressed().whenComplete(
                  () {
                    context.router.pop();
